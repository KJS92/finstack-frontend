...
const TransactionsList: React.FC = () => {
  ...
  const [confirmBulkCategorize, setConfirmBulkCategorize] = useState(false);
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      // Auth + display name — single call, no redundant checkUser
  ...
  }, [selectedAccount, accounts, loadTransactions]);
  ...
  // Inline confirmation for bulk-categorize — replaces window.confirm
  const handleBulkCategorize = async () => {
    if (!confirmBulkCategorize) { setConfirmBulkCategorize(true); return; }
    setConfirmBulkCategorize(false);
    setBulkCategorizing(true);
    try {
      const uncategorized = transactions.filter(t => !t.category_id);
      if (uncategorized.length === 0) {
        setToast({ message: 'All transactions are already categorized!', type: 'info' }); return;
      }
  ...
  return (
    <div className="transactions-list-container">
      <AppHeader title="Transaction List" userEmail={userEmail} displayName={displayName} activePage="transactions" />
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', margin: '0 20px 16px' }}>
        <div
          ...
          onKeyDown={e => (e.key === 'Enter' || e.key === ' ') && setFiltersOpen(o => !o)}
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontWeight: 600, fontSize: '15px', color: '#111', fontFamily: theme.fontFamily.base }}>Filters &amp; Sort</span>
            {activeFilterCount > 0 && (
              <span style={{ background: '#16A34A', color: '#fff', borderRadius: '999px', fontSize: '11px', fontWeight: 700, padding: '2px 8px', fontFamily: theme.fontFamily.base }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            {activeFilterCount > 0 && (
              <button onClick={e => { e.stopPropagation(); clearAllFilters(); }} style={{ display: 'flex', alignItems: 'center', gap: '4px', background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: '8px', fontSize: '12px', fontWeight: 600, padding: '4px 10px', cursor: 'pointer', fontFamily: theme.fontFamily.base }}>
                <X size={12} /> Clear all
              </button>
            )}
        ...
        {filtersOpen && (
          <div id="filter-panel" style={{ padding: '0 20px 20px', borderTop: '1px solid #f3f4f6' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  {(['date', 'amount', 'description'] as SortField[]).map(f => (
                    <button key={f} onClick={() => handleSortChange(f)} style={{ flex: 1, padding: '8px 6px', border: '1px solid', borderColor: sortField === f ? '#16A34A' : '#ddd', borderRadius: '8px', background: sortField === f ? '#DCFCE7' : '#fff', color: sortField === f ? '#15803D' : '#555', fontSize: '12px', fontWeight: sortField === f ? 700 : 400, cursor: 'pointer', textTransform: 'capitalize', fontFamily: theme.fontFamily.base }}>
      ...
      <div className="view-toggle">
        ...
        {/* Auto-Categorize with inline confirmation */}
        {confirmBulkCategorize ? (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '13px', color: '#0F172A', fontFamily: theme.fontFamily.base }}>
            Categorize all?
            <button onClick={handleBulkCategorize} style={{ padding: '6px 12px', background: '#0F172A', color: '#fff', border: 'none', borderRadius: '6px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: theme.fontFamily.base }}>Yes</button>
            <button onClick={() => setConfirmBulkCategorize(false)} style={{ padding: '6px 12px', background: 'none', border: '1px solid #e5e7eb', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', color: '#64748B', fontFamily: theme.fontFamily.base }}>No</button>
          </span>
        ) : (
          <button
            onClick={handleBulkCategorize}
            disabled={bulkCategorizing}
            style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 16px', background: bulkCategorizing ? '#e5e7eb' : '#0F172A', color: bulkCategorizing ? '#999' : '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 600, cursor: bulkCategorizing ? 'not-allowed' : 'pointer', fontFamily: theme.fontFamily.base }}
          >
            <Sparkles size={14} />
                      ...
                      <td>
                        <div className="action-buttons">
                          ...
                          {confirmDeleteId === txn.id ? (
                            <>
                              <button onClick={() => handleDelete(txn.id)} className="btn-confirm-delete-yes">Yes</button>
                              <button onClick={() => setConfirmDeleteId(null)} className="btn-confirm-delete-no">No</button>
                            </>
                          ) : (
...
const FilterChip: React.FC<{ label: string; onRemove: () => void }> = ({ label, onRemove }) => (
  <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: '#F0FDF4', color: '#15803D', border: '1px solid #BBF7D0', borderRadius: '999px', fontSize: '12px', fontWeight: 500, padding: '3px 10px', fontFamily: theme.fontFamily.base }}>
    {label}
...
const filterLabelStyle: React.CSSProperties = { display: 'block', fontSize: '12px', fontWeight: 600, color: '#555', marginBottom: '6px', fontFamily: theme.fontFamily.base };
const filterInputStyle: React.CSSProperties = { width: '100%', padding: '8px 10px', border: '1px solid #ddd', borderRadius: '8px', fontSize: '13px', color: '#333', background: '#fafafa', boxSizing: 'border-box', fontFamily: theme.fontFamily.base };
