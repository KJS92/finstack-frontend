...
class AssetsService {
  ...
  async getAll(): Promise<Asset[]> {
    ...
    const { data, error } = await supabase.from('assets').select('*').eq('user_id', user.id).eq('is_active', true).order('created_at', { ascending: false });
  ...
  async getByType(type: 'investment' | 'insurance'): Promise<Asset[]> {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('Not authenticated');
    const { data, error } = await supabase.from('assets').select('*').eq('user_id', user.id).eq('type', type).eq('is_active', true).order('created_at', { ascending: false });
    if (error) throw error;
    return data || [];
  }
  ...
  async delete(id: string): Promise<void> {
    const { error } = await supabase.from('assets').update({ is_active: false, updated_at: new Date().toISOString() }).eq('id', id);
    if (error) throw error;
  }
  ...
  async getSummary(): Promise<AssetSummary> {
    ...
    const [assetsResult, accountsResult, rpResult] = await Promise.all([
      ...
      supabase.from('receivables_payables').select('*').eq('user_id', user.id).neq('status', 'completed').eq('is_recurring', false),
    ]);
    ...
    assetsResult.data?.forEach(asset => {
      if (asset.type === 'investment') {
        ...
        if (asset.invested_amount) summary.totalGainLoss += asset.current_value - asset.invested_amount;
      } else {
        summary.totalInsurance += asset.current_value;
      }
      if (asset.maturity_date) {
        const maturityDate = new Date(asset.maturity_date);
        if (maturityDate >= today && maturityDate <= reminderDate) summary.upcomingMaturities.push(asset);
      }
    });

    accountsResult.data?.forEach(account => {
      if (account.type === 'credit_card') summary.totalLiabilities += account.balance || 0;
      else summary.totalBankBalance += account.balance || 0;
    });

    rpResult.data?.forEach(entry => {
      if (entry.type === 'receivable') summary.totalReceivables += entry.remaining_amount;
      else summary.totalPayables += entry.remaining_amount;
    });

    summary.totalAssets = summary.totalInvestments + summary.totalInsurance + summary.totalBankBalance + summary.totalReceivables;
  ...
  getCategoryLabel(category: string): string {
    const labels: Record<string, string> = {
      fd: 'Fixed Deposit', sip: 'SIP / Mutual Fund', stocks: 'Stocks',
      mutual_fund: 'Mutual Fund', ppf: 'PPF', nps: 'NPS', gold: 'Gold',
      real_estate: 'Real Estate', other_investment: 'Other Investment',
      life_insurance: 'Life Insurance', health_insurance: 'Health Insurance',
      vehicle_insurance: 'Vehicle Insurance', term_insurance: 'Term Insurance',
      other_insurance: 'Other Insurance'
    };
    return labels[category] || category;
  }

  // getCategoryIcon kept for any legacy callers but now returns empty string
  // UI should use CATEGORY_ICON_MAP in Assets.tsx instead
